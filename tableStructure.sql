CREATE TABLE admin (
    ...
    role_name     NVARCHAR2(200) DEFAULT 'admin' CHECK (role IN ('admin', 'response-admin', 'audit-admin', 'special-admin', 'super-admin')) NOT NULL,
);
-- visitor, response-admin, audit-admin, special-admin, super-admin
-- user table is in a another database
...
CREATE TABLE response (
        ...
        current_status          NVARCHAR2(200) DEFAULT N'Ирсэн' CHECK (current_status IN (N'Ирсэн', N'Ажиллаж байгаа', N'Шийдэгдсэн', N'Татгалзсан', N'Шалгаж байгаа', N'Зөвшөөрсөн', N'Буцаасан')) NOT NULL,
...
BEGIN
    -- Get the hazard type name (English) and increment the last_index
    SELECT name_en, NVL(last_index, 0) + 1
...
FOR EACH ROW
BEGIN
    IF :NEW.current_status = N'Зөвшөөрсөн' THEN
